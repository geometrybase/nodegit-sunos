...
Repository.discover = function (startPath, acrossFs, ceilingDirs, callback) {
  return discover(startPath, acrossFs, ceilingDirs).then(function (foundPath) {
 ...
 */
Repository.prototype.mergeBranches = function (to, from, signature, mergePreference, mergeOptions) {
  var repo = this;
  var fromBranch;
  var toBranch;
    ...
    return NodeGit.Merge.base(repo, toCommitOid, fromCommitOid).then(function (baseCommit) {
      if (baseCommit.toString() == fromCommitOid) {
        ...
        return NodeGit.Reference.lookup(repo, "HEAD").then(function (headRef) {
          ...
          var message = "Merged " + fromBranch.shorthand() + " into " + toBranch.shorthand();
 ...
 */
Repository.prototype.rebaseBranches = function (branch, upstream, onto, signature, beforeNextFn) {
  var repo = this;
  var branchCommit;
  var upstreamCommit;
  var ontoCommit;
  ...
  return Promise.all([repo.getReference(branch), upstream ? repo.getReference(upstream) : null, onto ? repo.getReference(onto) : null]).then(function (refs) {
    ...
    if (oid.toString() === branchCommit.id().toString()) {
      ...
      return repo.mergeBranches(branch, upstream).then(function () {
    ...
    return NodeGit.Rebase.init(repo, branchCommit, upstreamCommit, ontoCommit).then(function (rebase) {
 ...
 * @return {Number} 0 or an error code
 */
Repository.prototype.stageLines = function (filePath, selectedLines, isSelectionStaged) {
